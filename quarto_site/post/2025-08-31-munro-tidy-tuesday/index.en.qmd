...
There are 282 Munros, but these are covered by 161 walkhighlands routes.

```{r}
route_dat <-munro_dat |>
  select(first_route_title, region, time_hours_min:deer_fence, time, scramble_exposed, wet) |>
  unique()
```
...
for_sandie <- munros_coords |>
  pivot_longer(cols = toilet:car_park, names_to = "feature", values_to = "present")

p4 <- ggplot() +
  geom_sf(data = uk_map, fill = "lightgray", color = "darkgrey", size = 0.3) +
  coord_sf(xlim = c(-8, -1.5), ylim = c(56.5, 58.6)) +
  geom_jitter(data = for_sandie, 
             aes(x = longitude, 
                 y = latitude, 
                 colour = feature,
                 text = munro), 
  ...
  scale_x_continuous(breaks = NULL) +
  scale_y_continuous(breaks = NULL) +
  scale_colour_manual(values = nature_5) +
  labs(title = "Non-natural features",
              subtitle = "Walk descriptions that reference:",
       colour = "Route mentions") +
  theme_economist() +
  theme(
