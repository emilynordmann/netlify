...
route_dat <-munro_dat |>
  select(first_route_url, first_route_title,
         region, time_hours_min:bike, time, scramble_exposed, wet) |>
  unique()
...
munros_map <- munro_dat %>%
  select(munro, region, xcoord, ycoord, height_dobih, scramble_exposed, wet, toilet, pub, bothy, deer_fence, car_park, cow, bike) %>%
  na.omit()
...
# Convert to regular polygon data frame for ggplotly compatibility.
# ggplotly cannot handle the sfc geometry list column that geom_sf carries,
# so we use geom_polygon + coord_cartesian for all interactive maps.
uk_map_df <- uk_map %>%
...
p <- ggplot() +
  geom_polygon(data = uk_map_df, aes(x = X, y = Y, group = group),
               fill = "lightgray", color = "darkgrey", linewidth = 0.3) +
  coord_cartesian(xlim = c(-8, -1.5), ylim = c(56.5, 58.6)) +
...
ggplot(
  ...
  aes(
    x = height_dobih,
    y = fct_reorder(region, height_dobih, .fun = \(x) mean(x, na.rm = TRUE), .desc = FALSE),
    fill = region
  )
)  +
  geom_density_ridges(
...
ggplot(
  ...
  aes(
    ...
    y = fct_reorder(region, distance_km, .fun = \(x) mean(x, na.rm = TRUE), .desc = FALSE),
    fill = region
  )
) +
  geom_density_ridges(
...
ggplot(
  ...
  aes(
    ...
    y = fct_reorder(region, ascent, .fun = \(x) mean(x, na.rm = TRUE), .desc = FALSE),
    fill = region
  )
) +
  geom_density_ridges(
...
ggplot(
  ...
  aes(
    x = rating,
    y = fct_reorder(region, rating, .fun = \(x) mean(x, na.rm = TRUE), .desc = FALSE),
    fill = region
  )
)  +
  geom_density_ridges(
    quantile_lines = TRUE, quantile_fun = mean,
    vline_linetype = "dashed",
    aes(colour = "Mean rating")
  ) +
  scale_y_discrete(expand = c(0.01, 0)) +
  scale_x_continuous(expand = c(0.01, 0)) +
  scale_colour_manual(values = c("Mean rating" = "black")) +
  theme_economist() +
...
With the usual caveats about the limits of text mining, here's some maps showing route descriptions that mention other features (cows (scary), bothy, deer fence (scary), pub, toilet). Some of these are more useful than others. 

### Cows

Which routes mention cows, cattle, or calves? With the usual caveats about text-mining: "cow" can appear in place names (Cow Hill, for instance) as well as route descriptions, so treat this as a rough guide rather than a definitive livestock census.

```{r}
p_cow <- ggplot() +
  geom_polygon(data = uk_map_df, 
  ...
  labs(title = "Where are the cows?",
       subtitle = "Route descriptions that mention cows, cattle, or calves",
       colour = NULL) +
  theme_economist() +
  theme(
    axis.text = element_blank(),
...
ggplotly(p_cow, tooltip = "text")
```

### Bikes

Which routes mention bikes or cycling?

```{r}
bike_plot <- ggplot() +
  geom_polygon(data = uk_map_df, aes(x = X, y = Y, group = group),
               fill = "lightgray", color = "darkgrey", linewidth = 0.3) +
  coord_cartesian(xlim = c(-8, -1.5), ylim = c(56, 58.6)) +
  geom_jitter(data = filter(munros_coords, bike == TRUE), 
             aes(x = longitude, 
                 y = latitude, 
                 colour = bike,
                 text = munro), 
  ...
  scale_x_continuous(breaks = NULL) +
  scale_y_continuous(breaks = NULL) +
  labs(title = "Routes that mention a bike") +
  guides(colour = "none") +
  theme_economist() +
  theme(
    axis.text = element_blank(),      
...
ggplotly(bike_plot, tooltip = "text")
```
