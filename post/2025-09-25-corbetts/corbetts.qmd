...
As in my Munro adventure, the Corbett analysis combines two sources of data - the [Database of British and Irish Hills v18.2](https://www.hills-database.co.uk/) and [walkhighlands](https://www.walkhighlands.co.uk/). I previously obtained walkhighlands permission to use the data like this and as always, if you use walkhighlands, you should give them [some money](https://www.walkhighlands.co.uk/donate.shtml). 
...
route_dat <-corbett_dat |>
  select(first_route_url, first_route_title,
         region, time_hours_min:bike, time, scramble_exposed, wet, type) |>
  unique()
...
```{r}
#| tbl-cap: "Percent of route descriptions that contain scary words"
...
corbett_map <- corbett_dat %>%
  select(name, region, xcoord, ycoord, height_dobih, pathless, scramble_exposed, wet, toilet, pub, bothy, deer_fence, car_park, cow, bike) %>%
  na.omit()
...
Routes that mention a bike or cycling
...
bike_plot <- ggplot() +
  geom_polygon(data = uk_map_df, aes(x = X, y = Y, group = group),
               fill = "lightgray", color = "darkgrey", linewidth = 0.3) +
  coord_cartesian(xlim = c(-8, -1.5), ylim = c(56, 58.6)) +
  geom_jitter(data = filter(corbett_coords, bike == TRUE),
             aes(x = longitude,
                 y = latitude,
                 colour = bike,
                 text = name),
  ...
  scale_x_continuous(breaks = NULL) +
  scale_y_continuous(breaks = NULL) +
  labs(title = "Corbett routes that mention a bike") +
  guides(colour = "none") +
  theme_economist() +
  theme(
    axis.text = element_blank(),
...
Cows are terrifying  - which Corbett routes mention cows, cattle, or calves? With the usual text-mining caveats: "cow" can appear in place names as well as route descriptions.
